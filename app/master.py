...
def list_metadata_to_dataframe(data_list, format):
...
def read_files(path):
        ...
        if os.path.isfile(file_path):
            ...
            match extension.suffix:
                case ".xlsx":
                    ...
                    list_data_unified.append(['xlsx'] + [file] + list(dict_xls.values()))
                    list_xls.append(dict_xls)
                case ".docx":
                    dict_doc = extract_doc(file_path)
                    list_data_unified.append(['docx'] + [file] + list(dict_doc.values()))
                    list_doc.append(dict_doc)
                case ".pptx":
                    dict_ppt = extract_ppt(file_path)
                    list_data_unified.append(['pptx'] + [file] + list(dict_ppt.values()))
                    list_ppt.append(dict_ppt)
                case ".pdf":
                    dict_pdf = extract_pdf(file_path)
                    list_data_unified.append(['pdf'] + [file] + list(dict_pdf.values()))
    ...
    df_data_unified = pd.DataFrame(data=list_data_unified, columns=['Type', 'Filename', 'Titulo', 'Autor', 'Asunto', 'Palabras_Clave', 'File_Size', 'Fecha_Creacion', 'Fecha_Modificacion', 'Categoria', 'Comentarios', 'Numero_Paginas'])
